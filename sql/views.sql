...
AS
 SELECT row_number() OVER () AS id,
   ...
   FROM ( SELECT planet_osm_line.ref,
          ...
          GROUP BY planet_osm_line.highway, planet_osm_line.ref
        UNION
         SELECT ''::text AS ref,
            planet_osm_line.highway,
...
CREATE INDEX bikemap_highways_geom_idx
    ...
    (geom)
    TABLESPACE pg_default;
CREATE UNIQUE INDEX bikemap_highways_id
   ...
   FROM ( SELECT st_centroid(unnest(st_clusterwithin(aaa.way, 1000::double precision))) AS way,
           ...
           FROM ( SELECT planet_osm_point.way,
                        CASE
                            ...
                            WHEN planet_osm_point.amenity = 'shelter'::text OR planet_osm_point.tourism = 'alpine_hut'::text OR planet_osm_point.highway = 'bus_stop'::text AND planet_osm_point.tags @> '"shelter"=>"yes"'::hstore THEN 'shelter'::text
                ...
                UNION
                 SELECT st_centroid(planet_osm_line.way) AS way,
                        CASE
                            ...
                            WHEN planet_osm_line.amenity = 'shelter'::text OR planet_osm_line.tourism = 'alpine_hut'::text OR planet_osm_line.highway = 'bus_stop'::text AND planet_osm_line.tags @> '"shelter"=>"yes"'::hstore THEN 'shelter'::text
                ...
                UNION
                 SELECT st_centroid(planet_osm_polygon.way) AS way,
                        CASE
                            ...
                            WHEN planet_osm_polygon.amenity = 'shelter'::text OR planet_osm_polygon.tourism = 'alpine_hut'::text OR planet_osm_polygon.highway = 'bus_stop'::text AND planet_osm_polygon.tags @> '"shelter"=>"yes"'::hstore THEN 'shelter'::text
